...
public class ChatActivity extends ActionBarActivity{
	...
	public void onResume(){
		...
		Utility.RaiseToast(getApplicationContext(), "resume "+receiver_id+" "+sender_id, false);
	...
	public void fetchMessages(){
		...
		params.put(Constants.QueryParameters.TOKEN, spref.getString(Constants.PreferenceKeys.token,null));
		
		AsyncHttpClient client=new AsyncHttpClient();
		client.get("http://"+Constants.SERVER+Constants.COLON+Constants.PORT+"/SGI_webservice/query/download_messages",params, new JsonHttpResponseHandler(){
			@Override
			public void onSuccess(int statusCode,Header[] headers,JSONArray response){
				Utility.log(TAG,response.toString());
				//insert into db
				if(response.length()>0){
					new DbHelper(getApplicationContext()).fillMessages(response,sender_id);
					...
					for(int i=0;i<len;i++){
						try {
							ack.put(((JSONObject)response.get(i)).getInt(Constants.JSONMEssageKeys.ID));
			...
			public void onFailure(int statusCode,Header[] headers,Throwable throwable,JSONObject errorResponse){
				Utility.log(TAG,""+errorResponse.toString());
			}
	...
	public void sendAckForMessages(JSONArray ids){
		...
		client.get("http://"+Constants.SERVER+Constants.COLON+Constants.PORT+"/SGI_webservice/query/receive_ack",params, new JsonHttpResponseHandler(){
			@Override
	...
	public void sendMessage(View view){
		...
		if(msgtxt.trim().length()>0){
			...
			values.put(DbStructure.MessageTable.COLUMN_TIME,Calendar.getInstance().getTimeInMillis());//(String)DateUtils.getRelativeDateTimeString(getApplicationContext(), cal.getTimeInMillis(),  DateUtils.SECOND_IN_MILLIS, DateUtils.WEEK_IN_MILLIS, 0));
			...
			msg_id=db.insert(DbStructure.MessageTable.TABLE_NAME, null, values);
		...
		//	Utility.RaiseToast(getApplicationContext(), "send", false);
			...
			try {
				...
				mobj.put(Constants.JSONMEssageKeys.TIME, Calendar.getInstance().getTimeInMillis());
			...
			msg.setText("");
			AsyncHttpClient client=new AsyncHttpClient();
			client.get("http://"+Constants.SERVER+Constants.COLON+Constants.PORT+"/SGI_webservice/query/upload_message",params, new JsonHttpResponseHandler(){
				@Override
				public void onSuccess(int statusCode,Header[] headers,JSONObject response){
					Utility.log(TAG,response.toString());
					//if status is false then reset message state to send again id in msg_id
					//update messages 
					try{
						if(response.getBoolean(Constants.JSONKeys.STATUS)){
							ContentValues values=new ContentValues();
							values.put(DbStructure.MessageTable.COLUMN_STATE, Constants.MsgState.SENT_SUCESSFULLY);
							new DbHelper(getApplicationContext()).getWritableDatabase()
							.update(DbStructure.MessageTable.TABLE_NAME, values, "_id=?", new String[]{
							msg_id+""		
							});
						}
					}
