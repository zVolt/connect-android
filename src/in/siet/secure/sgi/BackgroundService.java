...
public class BackgroundService extends Service {
	...
	private class doPopo extends AsyncTask<Void, Void, Void> {
		...
		protected Void doInBackground(Void... params) {
			...
			client.get(Utility.BASE_URL + "query/download_messages", reqparams,
						...
						public void onSuccess(int statusCode, Header[] headers,
							...
							try {
								...
								if (response.length() > 0) {
									...
									sender_id = tmpobj
											.getString(Constants.JSONMEssageKeys.ID);
									sender_name = null;
									
									// sender db me se nikal le yaha
									String qry = "select f_name,l_name from user where login_id='"
											+ sender_id + "'";
									db = new DbHelper(getApplicationContext())
											.getWritableDatabase();
									Cursor c = db.rawQuery(qry, null);
									if (c.moveToFirst()) {
										sender_name = c.getString(0)
												+ c.getString(1);
									} else {
										sender_name = "Guddu"; // :D
									}
									...
									qry="select _id from user where login_id='"+user_id+"'";
									...
									c.close();
									new DbHelper(getApplicationContext()).fillMessages(response, mera_pk);																
									Utility.buildNotification(
											getApplicationContext(),
											ChatActivity.class,
											Constants.NOTIFICATION_DRAWER.NOTI_MESSAGE,
											null, null, sender_id, sender_name);
