	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView == null) {
			...
			holder.image = (ImageView) convertView
					.findViewById(R.id.imageViewListNotiImage);
			holder.subject = (TextView) convertView
					.findViewById(R.id.notification_list_title);
			holder.time = (TextView) convertView
					.findViewById(R.id.notification_list_time);
		} else {
			holder = (ViewHolder) convertView.getTag();
		}
		ImageLoader.getInstance().displayImage(values.get(position).image,
				holder.image);
		holder.subject.setText(values.get(position).subject);

		holder.time.setText(Utility.getTimeString(context,
				values.get(position).time));
